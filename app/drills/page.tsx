...
import { useSession } from "@/lib/session"; // Adjust path based on your session hook location
import { useRouter } from "next/navigation";
import { DAILY_DRILLS } from "@/lib/dummy-data";
...
export default function DrillsPage() {
  ...
  const completed = DAILY_DRILLS.filter((d) => done[d.id]).length;
  const pct = Math.round((completed / DAILY_DRILLS.length) * 100);
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:py-16">
      <header className="mb-8 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end">
        ...
        <div className="glass rounded-xl px-5 py-3 text-right">
          <div className="font-display text-3xl font-extrabold text-gradient-neon">{pct}%</div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
            {completed}/{DAILY_DRILLS.length} complete
          </div>
        </div>
      </header>
        ...
        {DAILY_DRILLS.map((d) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="font-display text-xl font-extrabold">{d.title}</h3>
              ...
              <div className="mt-4 flex gap-2">
                ...
                {isDone && (
                  <button
                    onClick={() => setDone((p) => ({ ...p, [d.id]: false }))}
        ...
        <div className="glass-strong mt-8 flex items-center gap-4 rounded-2xl p-6">
          ...
          <div>
            ...
            <p className="mt-1 font-display text-lg font-bold text-gradient-neon">
              "ALL DRILLS DOWN! That's a champion's day. Now go ranked and SHOW it!"
            </p>
          </div>
        </div>
      )}
